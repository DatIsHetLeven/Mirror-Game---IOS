    ...
    var body: some View {
        VStack {
            ...
            ScrollView {
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack {
                        ForEach(viewModel.groupedCards.keys.sorted(), id: \.self) { themeId in
                            Button(action: {
                                viewModel.selectedThemeId = viewModel.selectedThemeId == themeId ? nil : themeId
                            }) {
                                VStack {
                                    Text(viewModel.themeName(for: themeId))
                                        .font(.headline)
                                        .foregroundColor(.white)
                                }
                                .frame(width: 120, height: 120)
