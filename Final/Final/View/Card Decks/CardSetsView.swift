...
struct CardSetsView: View {
    ...
    var body: some View {
        ScrollView {
                    ...
                    NavigationLink(destination: CardSetDetailView(cardSetId: cardSet.id)) {
                        VStack(alignment: .leading) {
                            ...
                            HStack {
                                Spacer()
                                Button(action: {
                                    viewModel.selectedCardSetId = cardSet.id
            ...
            .onAppear(perform: viewModel.loadCardSets)
            .alert(isPresented: $viewModel.showingDeleteAlert) {
                Alert(
                    title: Text("Verwijderen bevestigen"),
                    message: Text("Weet je zeker dat je deze set wilt verwijderen?"),
                    primaryButton: .destructive(Text("Ja, verwijder")) {
                        if let cardSetId = viewModel.selectedCardSetId {
                            viewModel.deleteCardSet(cardSetId: cardSetId)
                        }
                    },
                    secondaryButton: .cancel()
                )
